            ...
            <div class="overflow-hidden rounded-3xl border border-amber-100 bg-white shadow-2xl">
                ...
                <div
                    ...
                    <div class="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                        <div>
                            ...
                            <p class="mt-3 max-w-2xl text-sm leading-6 text-zinc-200 sm:text-base">
                                Kelola data capster dengan tampilan yang lebih rapi, cepat, dan nyaman untuk operasional
                                barber shop.
                            </p>
                        </div>
                ...
                <div class="p-6 sm:p-8">
                    ...
                    <form method="GET" action="{{ route('capsters.index') }}"
                        ...
                        <div class="relative">
                            <i
                                class="fa-solid fa-magnifying-glass pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 text-zinc-400"></i>
                                ...
                                <thead class="bg-gradient-to-r from-zinc-900 to-amber-900">
                                    <tr>
                                        @foreach (['#', 'Foto', 'Nama', 'Inisial', 'Gender', 'No. HP', 'Asal', 'Status', 'Aksi'] as $col)
                                            <th
                                                class="px-4 py-4 text-left text-xs font-bold uppercase tracking-wider text-white">
                                                {{ $col }}
                                            </th>
                                        @endforeach
                                    </tr>
                                        ...
                                        <tr class="transition hover:bg-amber-50/60">
                                            ...
                                            <td class="px-4 py-4">
                                                @if (strtolower($capster->status) === 'aktif')
                                                    <span
                                                        class="inline-flex items-center rounded-full bg-emerald-500 px-3 py-1 text-xs font-bold text-white shadow-sm">
                                                        <i class="fa-solid fa-circle-check mr-1"></i> Aktif
                                                    </span>
                                                @else
                                                    <span
                                                        class="inline-flex items-center rounded-full bg-rose-500 px-3 py-1 text-xs font-bold text-white shadow-sm">
                                                        <i class="fa-solid fa-circle-xmark mr-1"></i> Keluar
                                                    </span>
                                                @endif
                                            </td>
                                        ...
                                        <tr>
                                            <td colspan="9" class="px-4 py-10 text-center text-sm text-zinc-500">
                                                <div class="flex flex-col items-center">
                                                    ...
                                                    <p class="mt-3 font-medium">No capsters found.</p>
                                                </div>
                                            </td>
                                        </tr>
                                    @endforelse
                    ...
                    <div class="md:hidden space-y-4">
                        @forelse($capsters as $capster)
                            <div
                                class="rounded-2xl border border-zinc-200 bg-white p-4 shadow-md transition hover:shadow-lg">
                                <div class="flex items-center justify-between gap-3">
                                    ...
                                    @if (strtolower($capster->status) === 'aktif')
                                        <span
                                            class="inline-flex items-center rounded-full bg-emerald-500 px-3 py-1 text-xs font-bold text-white">
                                            Aktif
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center rounded-full bg-rose-500 px-3 py-1 text-xs font-bold text-white">
                                            Keluar
                                        </span>
                                    @endif
                                </div>
                                ...
                                <div
                                    class="mt-4 space-y-2 rounded-xl bg-zinc-50 p-3 text-sm text-zinc-700 ring-1 ring-zinc-100">
                                    <p><span class="font-semibold text-purple-600">Gender:</span>
                                        {{ $capster->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan' }}</p>
                                    <p><span class="font-semibold text-sky-600">No. HP:</span> {{ $capster->no_hp ?? '-' }}
                                    </p>
                                    <p><span class="font-semibold text-pink-600">Asal:</span> {{ $capster->asal ?? '-' }}
                                    </p>
                                </div>
                        ...
                        @empty
                            <div
                                class="rounded-2xl border border-dashed border-zinc-300 bg-zinc-50 px-6 py-10 text-center text-zinc-500">
                                <i class="fa-solid fa-scissors text-3xl text-zinc-300"></i>
                                <p class="mt-3 font-medium">No capsters found.</p>
                            </div>
                        @endforelse
