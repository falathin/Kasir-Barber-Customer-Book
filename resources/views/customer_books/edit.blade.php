...
@section('title', 'Edit Customer Book')
            ...
            <div class="p-6">
                ...
                <form action="{{ route('customer-books.update', $customerBook->id) }}" method="POST" class="space-y-5"
                    ...
                    <label class="block">
                        ...
                        <div class="relative mt-1">
                            <input type="text" name="cap" value="{{ old('cap', $customerBook->cap) }}"
                        ...
                        <div class="relative mt-1">
                            <select name="haircut_type"
                                ...
                                <option value="" disabled
                                    ...
                                    --</option>
                                <option value="Gentle"
                                    {{ old('haircut_type', $customerBook->haircut_type) == 'Gentle' ? 'selected' : '' }}>
                                    Gentle</option>
                                <option value="Ladies"
                                    {{ old('haircut_type', $customerBook->haircut_type) == 'Ladies' ? 'selected' : '' }}>
                                    Ladies</option>
                            </select>
                        </div>
                        @error('haircut_type')
                    ...
                    <label class="block">
                        ...
                        <textarea name="colouring_other" placeholder="e.g. Pastel pink highlights"
                            ...
                            rows="3">{{ old('colouring_other', $customerBook->colouring_other) }}</textarea>
                    </label>
                        ...
                        <div class="relative mt-1">
                            <select name="qr"
                                ...
                                <option value="qr_transfer"
                                    {{ old('qr', $customerBook->qr) == 'qr_transfer' ? 'selected' : '' }}>QR Transfer
                                </option>
                            </select>
                        </div>
                    </label>
                    ...
                    <label class="block">
                        ...
                        <div class="relative mt-1">
                            <input type="text" name="barber_name"
                                value="{{ old('barber_name', $customerBook->barber_name) }}" placeholder="e.g. John Barber"
                                class="w-full pl-3 pr-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                required>
                        </div>
                        @error('barber_name')
                    ...
                    <button type="submit"
                        class="w-full py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition">Update</button>
                </form>
            </div>
        </div>
    </div>
